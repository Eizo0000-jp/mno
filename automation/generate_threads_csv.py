...
- 日常系カジュアル投稿（1日5回ペース、時間帯×平日/休日に合ったコンテンツ）
"""
...
CASUAL_BY_TIME = {
    ...
    "morning": {
        "weekday": [
            ...
            "今日も朝コーヒーからスタート☕ 最近コンビニのカフェラテにハマってる。じわじわおいしくなってる気がする",
            ...
            "6時に起きて朝ランしてきた。新緑の道を走るのが気持ちよくて早起きが苦じゃなくなってきた🌿",
        ],
        "weekend": [
            # 休日の朝：のんびり・朝活・休日ルーティン
            ...
            "休日の朝、ドリップコーヒーをゆっくり淹れて窓から外を眺める時間が好き☕ 平日にはできない贅沢",
            "今日は休みなのでゆっくり10時まで寝た。こんな贅沢な寝坊は週末だけの特権",
            "朝からヨガをやってみた。終わったあと体が軽くてびっくり。これは続けてみようかも🧘‍♀️",
        ],
    },
    ...
    "lunch": {
        "weekday": [
            ...
            "今日のランチ後に同僚と季節のスイーツ食べてきた🍰 限定って言われると食べないわけにいかない",
        ],
        "weekend": [
            # 休日のランチ：外食・友達・のんびりカフェ
            ...
            "休日ランチは時間を気にしないで食べられるのがいい。今日は気になってたカフェでゆっくり☕",
            "家族でランチ。近所においしいイタリアンができてたので行ってみた。パスタが絶品だった🍝",
            "地元の商店街をぶらぶらして、気になったお惣菜屋さんで食材調達🥦 休日ならではの楽しみ",
            "週末のランチは料理して食べるのが好き。今日は久しぶりにパスタを一から作った🍝",
        ],
    },
    ...
    "afternoon": {
        ...
        "weekend": [
            ...
            "美術館に行ってきた。たまにはこういう時間も必要だなってなった。非日常感がよかった🎨",
        ],
    },
    ...
    "evening": {
        "weekday": [
            ...
            "残業だったけど今日やりたかった仕事が全部片付いた。遅くなったけどすっきりして帰れる",
        ],
        "weekend": [
            # 休日夕方：外食・のんびり・お出かけ後
            ...
            "夕方の公園で散歩してきた。夕日の中の新緑がきれいすぎてしばらく佇んでしまった🌳",
            "今日は一日遊んで疲れたけどいい疲れ。夕ごはんは何も考えずにデリバリーで決まり",
            "週末の夕飯、久しぶりに凝った料理を作った。時間がある日じゃないとできないやつ🍳",
        ],
    },
    ...
    "night": {
        "weekday": [
            ...
            "夜のルーティン、ストレッチしながら今日あったことをぼんやり整理する時間が好き",
            "手帳を書くのが最近の習慣になってる。ただの日記だけど、振り返ると気づきがある📓",
            "夜に一人でコーヒー飲みながらぼーっとする時間。何も考えなくていい時間が一番回復できる気がする☕",
        ],
        "weekend": [
            # 休日夜：ゆっくり・日曜ブルー・のんびり
            ...
            "週末の夜は翌朝を気にしなくていいから、好きな時間まで起きていられるのがいい。今夜は映画を最後まで観た🎬",
            "今夜もいい一日だった。こういう充実した休日を過ごせるのって、やっぱり心と時間に余裕があってこそだな",
            "日曜の夜、来週の準備をしながらゆっくりお茶を飲む時間。これが一週間の締めくくり🍵",
        ],
    },
}
...
def build_schedule(start_date: date, all_articles: list[dict]) -> list[dict]:
    ...
    # 各スロット×曜日タイプのプールをシャッフル
    slot_pools: dict[str, dict[str, list]] = {}
    for slot, by_day in CASUAL_BY_TIME.items():
        slot_pools[slot] = {}
        for day_type, posts in by_day.items():
            pool = posts.copy()
            rng.shuffle(pool)
            slot_pools[slot][day_type] = pool
    slot_indices: dict[str, dict[str, int]] = {
        slot: {"weekday": 0, "weekend": 0} for slot in CASUAL_BY_TIME
    }
    ...
    for day_offset in range(SCHEDULE_DAYS):
        ...
        for hour, minute_base, slot in DAILY_TIMES:
            ...
            pool = slot_pools[slot][day_type]
            idx = slot_indices[slot][day_type]
            content = pool[idx % len(pool)]
            slot_indices[slot][day_type] += 1
